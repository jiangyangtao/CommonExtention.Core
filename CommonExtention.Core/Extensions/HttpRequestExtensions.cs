    ...
    public static class HttpRequestExtensions
    {
        ...
        public static string GetParamsString(this HttpRequest request)
        {
            try
            {
                ...
                if (method == "get" || method == "delete") return request.QueryString.Value;
                if (method == "post" || method == "put" || method == "patch")
                {
                    if (request.ContentType.Contains("multipart") && request.Form != null && request.Form.Count > 0)
                    {
                        if (request.Form.Files.Count > 0)
                        {
                            ...
                            {
                                resultBuilder.Append(files[i].FileName);
                                if (i != files.Count - 1) resultBuilder.Append(";");
                            }
                    ...
                    if (request.ContentType.Contains("json") && request.Body != null && request.Body.CanRead && request.Body.Length > 0)
                    {
